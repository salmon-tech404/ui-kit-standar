...
export const AiGenerateModal: React.FC<AiGenerateModalProps> = ({ isOpen, onClose, onOpenAuthModal }) => {
  ...
  const { updateToken, setTokens, tokens } = useDesignStore();
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl relative">
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {presetPrompts.map((preset) => (
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 text-slate-600 dark:text-slate-300 hover:text-indigo-600 text-xs rounded-lg border border-slate-200 dark:border-slate-700 transition"
                >
                  {preset.label}
                </button>
              ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Style Direction</label>
              <select
                value={style}
                onChange={(e) => setStyle(e.target.value)}
                className="w-full p-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none dark:text-white"
              >
                <option value="modern">Modern SaaS</option>
                <option value="minimalist">Minimalist</option>
                <option value="brutalist">Neo-Brutalist</option>
                <option value="enterprise">Enterprise Trust</option>
                <option value="vibrant">Vibrant Consumer</option>
              </select>
            </div>
            ...
            <div className="flex flex-col justify-end">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg shadow-md shadow-indigo-500/25 transition flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
